// Practica realizada por Alejandro Mu�oz Garcia y Roberto Gozalo Andres
...
public class Practica1_robgoza_alemuno {
		...
		try {
			...
			* variable auxiliar*/
			...
			while (true) {
				...
				// Se hace una copia del original y se busca con esa copia. En
				// la siguiente iteracion se vuelca el original a la copia y se
				// vuelve a buscar
				...
				for (clave = 0; clave < 65535; clave++) {
					...
					desofuscar(copia, clave);
					buscarCadena(copia, aux);
	...
	* proporcionado en el enunciado*/
	...
	// Metodo para buscar la cadena en el fichero proporcionado
	public static void buscarCadena(int copia[], int aux[]) {
		int n = copia.length;
		int m = aux.length;
		for (int i = 0; i < n - m; i++) {
			int j = 0;

			while (j < m && copia[i + j] == aux[j]) {
				j++;
			}
			if (j >= m) {
				System.out.println("cadena encontrada");
				mostrarCadena(i, copia);
			}

		}
	}

	/* Metodo que muestra al usuario los 100 caracteres anteriores a la cadena y
	*los 500 posteriores a el primer caracter de esta*/
	public static void mostrarCadena(int i, int copia[]) {
		...
		for (; k < i + 500; k++) {
			String imprimir = String.valueOf((char) copia[k]);
